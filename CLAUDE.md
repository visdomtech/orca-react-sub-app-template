...
│   ├── secured.ts     ← OrcaAgents path builder (orcaagents())
...
- Store and read all app-owned data via the **OrcaAgents DB API** using `orcaagents()` from `src/api/secured.ts` — never invent custom endpoints:
