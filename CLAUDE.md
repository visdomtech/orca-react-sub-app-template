...
- Use `secured()` / `securedv2()` from `src/api/secured.ts` **only** when calling existing Orca host backend endpoints (e.g. payroll, HR, leave — endpoints that already exist on the host)
